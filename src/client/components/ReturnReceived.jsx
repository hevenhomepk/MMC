...
export default function ReturnReceived({ shop }) {
  ...
  useEffect(() => {
    const fetchReturnReceived = async () => {
      setLoading(true);
      try {
        const resp = await fetch(`/api/returns/bookings?shop=${shop}&return_status=Return Received`);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.filterBar}>
        ...
        <select style={{ ...styles.input, minWidth: '160px' }} defaultValue="Return Received">
        ...
        <div style={{ position: 'relative' }}>
          ...
          {showActions && (
            <div style={{
              ...
              {['Sales Report', 'Item Report', 'Download Report'].map(opt => (
                <div 
                  key={opt}
            ...
            {loading ? (
              <tr>
                <td colSpan="13" style={{ ...styles.td, textAlign: 'center', padding: '48px', color: 'var(--muted)' }}>
                  Loading Return Received logs...
                </td>
              </tr>
            ) : filteredLogs.length === 0 ? (
              <tr>
                <td colSpan="13" style={{ ...styles.td, textAlign: 'center', padding: '48px', color: 'var(--muted)' }}>
                  No received return shipments found.
                </td>
              </tr>
            ) : (
              filteredLogs.map((b, i) => (
                <tr key={b.id}>
                  ...
                  <td style={styles.td}>
                    <span style={{ padding: '4px 10px', borderRadius: '99px', fontSize: '11px', fontWeight: '800', background: 'rgba(16,185,129,0.1)', color: '#10b981', textTransform: 'uppercase' }}>
                      {b.status}
                    </span>
                  </td>
